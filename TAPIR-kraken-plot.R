...
kraken_G230414 <- readr::read_tsv('Q:/IUK-A-MIGE/PROJECTS/TAPIR/files/combined_kraken_report_per_sequence_date/combined_kraken_report_S_G230414.txt')
kraken_G230414_unclassified <- readr::read_tsv('Q:/IUK-A-MIGE/PROJECTS/TAPIR/files/combined_kraken_report_per_sequence_date/combined_kraken_report_U_G230414.txt')

# merge
kraken_G230414_merge_df <- kraken_G230414 %>% bind_rows(kraken_G230414_unclassified)

head(kraken_G230414_merge_df) %>% kable() %>% kable_styling() %>% scroll_box(width = "100%")
...
kraken_G230414_modified_df <- kraken3_parse_df(kraken_G230414_merge_df)

head(kraken_G230414_modified_df) %>% kable() %>% kable_styling() %>% scroll_box(width = "100%")

#Show species contained in modified kraken report
kraken_G230414_species <- kraken_G230414_modified_df %>% 
  filter(!species %in% c('unclassified', 'unassigned', 'Others')) %>% distinct(species) %>% 
  pull() %>% unique() %>% sort()

kraken_G230414_species %<>% c('unclassified', 'unassigned', 'Others')

print(kraken_G230414_species)

print(current_species_name_and_colours$species_name)

#Identify species in the current df not present in the list of species
setdiff(kraken_G230414_species, current_species_name_and_colours$species_name)
...
kraken_G230414_species_name_and_colour <- current_species_name_and_colours %>% 
  filter(species_name %in% c(kraken_G230414_species)) %>% dplyr::pull(species_colour, species_name)
...
# filter only species of interest for the plot and convert dataframe to vector
kraken_G230414_species_name_and_colour <- current_species_names_and_colours_update1_df %>% 
  filter(species_name %in% c(kraken_G230414_species)) %>% dplyr::pull(species_colour, species_name)
...
kraken_G230414_plot <- plot_bar_kraken(kraken_G230414_modified_df, kraken_G230414_species_name_and_colour)
kraken_G230414_plot


write.table(kraken_G230414_modified_df, 'Q:/IUK-A-MIGE/PROJECTS/TAPIR/files/combined_kraken_report_per_sequence_date/G230414_combined_parsed_kraken_report.tsv', row.names = FALSE, sep = '\t')

jpeg("Q:/IUK-A-MIGE/PROJECTS/TAPIR/figures/G230414_kraken_plot.jpeg", width = 1000, height = 5000, units = 'px', res = 600)
print(kraken_G230414_plot)
dev.off()
